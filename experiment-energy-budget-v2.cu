/* experiment-energy-budget-v2.cu — Fixed: gentler ATP costs, agents survive
   ATP regeneration = 0.02/tick (up from 0.001), perception cost = 0.001 (down from 0.005)
   Tests if balanced biological constraints help or hurt. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,alive;
    float atp,max_atp,circadian_phase,tip_x,tip_y,tip_val;unsigned int rng;}Agent;
...
__global__ void init_bio(Agent*a,int n){
    ...
    a[i].max_atp=1.0f;a[i].atp=0.5f+lcgf(&a[i].rng)*0.5f;
    a[i].circadian_phase=lcgf(&a[i].rng)*6.28f;
}
...
__global__ void tick_bio(Agent*a,Resource*r,int na,int nr,int t,int pt){
    ...
    // Circadian: efficiency 60-100%
    float circ=sinf(t*0.015f+ag->circadian_phase)*0.2f+0.8f;
    // ATP costs (balanced: regen > total cost per tick)
    float perc_cost=0.001f;  // perception
    float move_cost=0.0005f; // movement
    float comm_cost=0.0003f; // communication
    float base_regen=0.002f; // base ATP regen
    ag->atp-=perc_cost; // always perceive
    // Detection/grab with circadian efficiency
    float det=(.03f+ep*.04f)*circ;
    float grab=(.02f+cp*.02f)*circ;
    ...
    if(br>=0&&bd<grab){
        ...
        float tb=1;for(int k=0;k<16;k++){int j=lcg(&ag->rng)%na;if(j==i||!a[j].alive||a[j].arch!=ag->arch)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y;if(sqrtf(dx*dx+dy*dy)<.05f)tb+=a[j].role[3]*.2f;}
        float bn=(1+cp*.5f)*tb;
        ag->energy=fminf(1,ag->energy+r[br].value*.1f*bn);ag->fitness+=r[br].value*bn;
        ag->atp=fminf(ag->max_atp,ag->atp+r[br].value*.15f); // food restores ATP
    }else if(br>=0){
        float dx=r[br].x-ag->x,dy=r[br].y-ag->y,d=sqrtf(dx*dx+dy*dy),sp=(.008f+cp*.008f+ep*.006f)*circ;
        ag->vx=ag->vx*.8f+(dx/d)*sp;ag->vy=ag->vy*.8f+(dy/d)*sp;
        ag->atp-=move_cost;
    }else{ag->vx=ag->vx*.95f+(lcgf(&ag->rng)-.5f)*.006f*(1+ep);ag->vy=ag->vy*.95f+(lcgf(&ag->rng)-.5f)*.006f*(1+ep);ag->atp-=move_cost*0.5f;}
    ag->x=fmodf(ag->x+ag->vx+1,1);ag->y=fmodf(ag->y+ag->vy+1,1);
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i||!a[j].alive)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);
        if(dist>=.06f)continue;ag->interactions++;ag->atp-=comm_cost;
    ...
    // Circadian regen
    ag->atp+=base_regen*circ;
    if(ag->atp>ag->max_atp)ag->atp=ag->max_atp;
    // Apoptosis only if ATP truly zero
    if(ag->atp<0){ag->alive=0;ag->atp=0;}
    // Starvation penalty: low ATP = reduced fitness gain (but no death)
    if(ag->atp<0.1f)ag->fitness*=0.99f;
    if(t==pt){ag->energy*=(1-.5f*(1-df*.5f));ag->x=lcgf(&ag->rng);ag->y=lcgf(&ag->rng);ag->vx=ag->vy=0;ag->tip_val=0;ag->atp=fminf(ag->max_atp,ag->atp+0.3f);}
}

__global__ void tick_norm(Agent*a,Resource*r,int na,int nr,int t,int pt){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;Agent*ag=&a[i];if(!ag->alive)return;
    float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    ...
    ag->x=fmodf(ag->x+ag->vx+1,1);ag->y=fmodf(ag->y+ag->vy+1,1);
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i||!a[j].alive)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);if(dist>=.06f)continue;ag->interactions++;
        if(a[j].role[2]>.5f&&cm>.2f){float jbd=.1f;int jbr=-1;for(int m=0;m<nr;m++){if(r[m].collected)continue;float mdx=r[m].x-a[j].x,mdy=r[m].y-a[j].y,md=sqrtf(mdx*mdx+mdy*mdy);if(md<jbd){jbd=md;jbr=m;}}if(jbr>=0){ag->tip_x=r[jbr].x;ag->tip_y=r[jbr].y;ag->tip_val=a[j].role[2];}}
        float infl=(a[j].arch==ag->arch)?.02f:.002f;for(int r=0;r<4;r++)ag->role[r]+=(a[j].role[r]-ag->role[r])*infl;
        float sim=0;for(int r=0;r<4;r++)sim+=1-fminf(1,fabsf(ag->role[r]-a[j].role[r]));sim/=4;if(sim>.9f){int dr=(ag->arch+1+lcg(&ag->rng)%3)%4;ag->role[dr]+=(lcgf(&ag->rng)-.5f)*.01f;}
        if(dist<.02f){ag->vx-=dx*.01f;ag->vy-=dy*.01f;}}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  Experiment 6v2: Energy Budget (balanced ATP)\n");
    printf("  A: bio (circadian 60-100%%, gentle costs)\n");
    printf("  B: normal specialists  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_bio=0,f_norm=0,f_ctrl=0;int total_dead=0;
    for(int e=0;e<5;e++){
        ...
        for(int i=0;i<NA;i++){f+=ha[i].fitness;if(!ha[i].alive)dead++;}f_bio+=f;total_dead+=dead;
        ...
        printf("Exp %d: bio=%.1f norm=%.1f ctrl=%.1f | dead=%d\n",e+1,f/NA,f/NA,f/NA,dead);
    }
    f_bio/=5;f_norm/=5;f_ctrl/=5;total_dead/=5;
    float ratio=(f_norm>.01f)?f_bio/f_norm:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Bio:    %.1f (%.2fx ctrl) avg_dead=%d\n",f_bio,(f_ctrl>.01f)?f_bio/f_ctrl:1,total_dead);
