/* v10: Does emergence scale with population size? */
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
int main(){
    printf("FLUX v10 — Population Scaling\n");
    printf("Agents: 256 512 1024 2048 4096 | 128 resources | 500 ticks\n\n");
    ...
    for(int si=0;si<5;si++){
        ...
        int blk=(na+255)/256,rblk=(N_RESOURCES+255)/256;
        float fs=0,fc=0;
        for(int e=0;e<3;e++){
            init_a<<<blk,256>>>(da,na);init_r<<<rblk,256>>>(dr,N_RESOURCES);cudaDeviceSynchronize();
            for(int t=0;t<MAX_TICKS;t++){tick_s<<<blk,256>>>(da,dr,na,N_RESOURCES,t,MAX_TICKS/2);cudaDeviceSynchronize();}
            cudaMemcpy(ha,da,na*sizeof(Agent),cudaMemcpyDeviceToHost);
            float f=0;for(int i=0;i<na;i++)f+=ha[i].fitness;fs+=f;
            init_c<<<blk,256>>>(db,na);init_r<<<rblk,256>>>(dr,N_RESOURCES);cudaDeviceSynchronize();
            for(int t=0;t<MAX_TICKS;t++){tick_c<<<blk,256>>>(db,dr,na,N_RESOURCES,t,MAX_TICKS/2);cudaDeviceSynchronize();}
        ...
        printf("N=%4d: spec=%.1f ctrl=%.1f ratio=%.2fx (%+.0f%%)\n",na,fs,fc,ratio,(ratio-1)*100);
