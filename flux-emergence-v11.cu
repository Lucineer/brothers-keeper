/* v11: How often can agents recover from perturbation? */
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
int main(){
    printf("FLUX v11 — Perturbation Frequency\n");
    printf("How often can specialized agents recover vs control?\n");
    printf("Intervals: never, 250, 125, 83, 50 ticks\n\n");
    int intervals[]={99999,250,125,83,50};
    const char*labels[]={"never","250","125","83","50"};
    ...
    for(int si=0;si<5;si++){
        ...
        for(int e=0;e<3;e++){
            ...
            for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,NA,NR,t,intervals[si]);cudaDeviceSynchronize();}
            ...
            for(int t=0;t<MAXT;t++){tick_c<<<blk,256>>>(db,dr,NA,NR,t,intervals[si]);cudaDeviceSynchronize();}
        ...
        printf("perturb=%s: spec=%.1f ctrl=%.1f ratio=%.2fx\n",labels[si],fs,fc,ratio);
