/* v12: Do specialists exploit diverse resources better?
   Uniform values vs power-law (few high-value, many low-value). */
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
__global__ void init_r_uniform(Resource*r,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);r[i].value=.5f+lcgf(&s)*.5f;r[i].collected=0;}

__global__ void init_r_powerlaw(Resource*r,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;unsigned int s=(unsigned int)(i*2654435761u+99999);r[i].x=lcgf(&s);r[i].y=lcgf(&s);/* Power law: first 10% are worth 5x, rest worth 0.2x */r[i].value=(i<n/10)?(.8f+lcgf(&s)*.2f):(.1f+lcgf(&s)*.2f);r[i].collected=0;}
...
int main(){
    printf("FLUX v12 — Resource Distribution\n");
    printf("Uniform vs Power-law (10%% high-value)\n\n");
    int blk=(NA+255)/256,rblk=(NR+255)/256;
    const char*dists[]={"uniform","powerlaw"};
    for(int di=0;di<2;di++){
        ...
        float fs=0,fc=0;
        for(int e=0;e<3;e++){
            init_a<<<blk,256>>>(da,NA);
            if(di==0)init_r_uniform<<<rblk,256>>>(dr,NR);
            else init_r_powerlaw<<<rblk,256>>>(dr,NR);
            ...
            float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;fs+=f;
            init_c<<<blk,256>>>(db,NA);
            if(di==0)init_r_uniform<<<rblk,256>>>(dr,NR);
            else init_r_powerlaw<<<rblk,256>>>(dr,NR);
            cudaDeviceSynchronize();
            for(int t=0;t<MAXT;t++){tick_c<<<blk,256>>>(db,dr,NA,NR,t,250);cudaDeviceSynchronize();}
            cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
            f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;fc+=f;
        }
        fs/=3;fc/=3;
        float ratio=(fc>.01f)?fs/fc:1;
        printf("dist=%-8s: spec=%.1f ctrl=%.1f ratio=%.2fx (%+.0f%%)\n",dists[di],fs,fc,ratio,(ratio-1)*100);
