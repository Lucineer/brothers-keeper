/* flux-emergence-v14.cu — Defensive Communication Specialization
   From DeepSeek hypothesis: agents broadcast AFTER defending, not on detection.
   Prediction: 15-25% fitness advantage over v8 in scarcity. */
...
__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].near_res=0;for(int r=0;r<4;r++)a[i].role[r]=.25f;}

/* v14: secure-then-share — communicators only broadcast when a defender is nearby */
__global__ void tick_v14(Agent*a,Resource*r,int na,int nr,int t,int pt){
    ...
    for(int j=0;j<nr;j++){if(r[j].collected)continue;float dx=r[j].x-ag->x,dy=r[j].y-ag->y,d=sqrtf(dx*dx+dy*dy);if(d<.1f)ag->near_res=1;if(d<bd){bd=d;br=j;}}
    ...
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);if(dist>=.06f)continue;ag->interactions++;
        /* v14: only broadcast if a defender (arch==3) is nearby */
        int has_defender=(a[j].role[3]>.4f&&a[j].near_res);
        if(a[j].role[2]>.5f&&cm>.2f&&has_defender){float jbd=.1f;int jbr=-1;for(int m=0;m<nr;m++){if(r[m].collected)continue;float mdx=r[m].x-a[j].x,mdy=r[m].y-a[j].y,md=sqrtf(mdx*mdx+mdy*mdy);if(md<jbd){jbd=md;jbr=m;}}if(jbr>=0){ag->tip_x=r[jbr].x;ag->tip_y=r[jbr].y;ag->tip_val=a[j].role[2];}}
    ...
    int dom=0;float dv=ag->role[0];for(int r=1;r<4;r++)if(ag->role[r]>dv){dv=ag->role[r];dom=r;}if(dom==ag->arch)ag->energy=fminf(1,ag->energy+.0005f);else ag->energy*=.9995f;ag->energy*=.999f;
...
__global__ void tick_v8(Agent*a,Resource*r,int na,int nr,int t,int pt){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float det=.03f+ep*.04f,grab=.02f+cp*.02f;
    float bd=det;int br=-1;
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v14 — Defensive Communication Specialization\n");
    printf("  A: v14 (secure-then-share, defender-gated broadcast)\n");
    printf("  B: v8 (immediate broadcast, baseline)\n");
    printf("  Prediction: v14 > v8 by 15-25%%\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float fv14=0,fv8=0,fc=0;
    for(int e=0;e<N_EXP;e++){
        init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_v14<<<blk,256>>>(da,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
        float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;fv14+=f;

        init_a<<<blk,256>>>(db,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_v8<<<blk,256>>>(db,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
        f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;fv8+=f;
        ...
        f=0;for(int i=0;i<NA;i++)f+=hc[i].fitness;fc+=f;
    }
    fv14/=N_EXP;fv8/=N_EXP;fc/=N_EXP;
    printf("v14 (secure-share): %.1f (vs ctrl: %.2fx)\n",fv14,fc>.01f?fv14/fc:1);
    printf("v8  (immediate):    %.1f (vs ctrl: %.2fx)\n",fv8,fc>.01f?fv8/fc:1);
    printf("v14/v8:             %.2fx\n",fv8>.01f?fv14/fv8:1);
    if(fv14>fv8*1.1) printf("VERDICT: CONFIRMED — secure-then-share wins\n");
    else if(fv14<fv8*.9) printf("VERDICT: FALSIFIED — gating reduces communication value\n");
    else printf("VERDICT: NEUTRAL — no significant difference\n");
