/* flux-emergence-v15.cu — Dynamic Anti-Convergence Threshold
   From DeepSeek hypothesis: adaptive similarity threshold (0.9→variable).
   Tighter when abundant, looser when scarce. */
...
__global__ void tick_dynamic(Agent*a,Resource*r,int na,int nr,int t,int pt,float thresh){
    ...
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);if(dist>=.06f)continue;ag->interactions++;
        ...
        if(sim>thresh){int dr=(ag->arch+1+lcg(&ag->rng)%3)%4;ag->role[dr]+=(lcgf(&ag->rng)-.5f)*.01f;}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v15 — Dynamic Anti-Convergence Threshold\n");
    printf("  Fixed 0.9 vs dynamic (based on resource scarcity)\n");
    printf("  Prediction: dynamic > fixed by 30%% spec\n");
    printf("═══════════════════════════════════════════════════════\n\n");

    int blk=(NA+255)/256,rblk=(NR+255)/256;
    float threshes[]={.7f,.8f,.85f,.9f,.95f};
    const char*labels[]={"0.70","0.80","0.85","0.90(fixed)","0.95"};
    for(int ti=0;ti<5;ti++){
        ...
        for(int e=0;e<3;e++){
            ...
            for(int t=0;t<MAXT;t++){tick_dynamic<<<blk,256>>>(da,dr,NA,NR,t,250,threshes[ti]);cudaDeviceSynchronize();}
            cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
            float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;fit+=f;sp+=spec(ha,NA);
        }
        fit/=3;sp/=3;
        printf("thresh=%-10s: fitness=%.1f spec=%.3f\n",labels[ti],fit,sp);
