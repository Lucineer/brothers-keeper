/* flux-emergence-v16.cu — Role Inheritance with Mutation (fixed) */
...
__global__ void tick_inh(Agent*a,Resource*r,int na,int nr,int t,int pt){
    ...
    if(t>0&&t%GEN==0){
        float bf=ag->fitness;int bj=i;
        for(int k=0;k<64;k++){int j=lcg(&ag->rng)%na;if(j!=i&&a[j].arch==ag->arch&&a[j].fitness>bf){bf=a[j].fitness;bj=j;}}
        if(bj!=i)for(int r=0;r<4;r++){ag->role[r]=a[bj].role[r]+(lcgf(&ag->rng)-.5f)*.05f;if(ag->role[r]<0)ag->role[r]=0;if(ag->role[r]>1)ag->role[r]=1;}
    }
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v16 — Role Inheritance with Mutation\n");
    printf("  Every %d ticks: copy fittest same-arch + 5%% mutation\n",GEN);
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float finh=0,fs=0,fc=0,si=0,ss=0;
    for(int e=0;e<N_EXP;e++){
        init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_inh<<<blk,256>>>(da,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
        float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;finh+=f;si+=spec(ha,NA);

        init_a<<<blk,256>>>(db,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(db,dr,NA,NR,t,250);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);
        f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;fs+=f;ss+=spec(hb,NA);
    ...
    finh/=N_EXP;fs/=N_EXP;fc/=N_EXP;si/=N_EXP;ss/=N_EXP;
    printf("Inherited:  fit=%.1f spec=%.3f (vs ctrl %.2fx)\n",finh,si,fc>.01f?finh/fc:1);
    printf("No-inherit: fit=%.1f spec=%.3f (vs ctrl %.2fx)\n",fs,ss,fc>.01f?fs/fc:1);
    printf("Control:    fit=%.1f\n",fc);
    if(si>ss+.05) printf("VERDICT: CONFIRMED — inheritance improves specialization\n");
    else if(finh>fs*1.1) printf("VERDICT: PARTIAL — better fitness but not spec\n");
    else printf("VERDICT: FALSIFIED — inheritance doesn't help\n");
