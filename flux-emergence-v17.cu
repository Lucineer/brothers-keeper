/* flux-emergence-v17.cu — Agent-to-Resource Ratio Deep Dive
   v10 showed N=512 → 85% advantage but N=1024 → 61%. Why?
   Fix agents=512, sweep resources: 32, 64, 128, 256, 512
   Find the optimal scarcity level. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt){
    ...
    if(t==pt){ag->energy*=(1-.5f*(1-df*.5f));ag->x=lcgf(&ag->rng);ag->y=lcgf(&a->rng);ag->vx=ag->vy=0;ag->tip_val=0;}
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v17 — Agent/Resource Ratio Sweep\n");
    printf("  Fixed 512 agents, sweep resources 16-512\n");
    printf("═══════════════════════════════════════════════════════\n\n");

    int nr_list[]={16,32,64,128,256,512};
    int blk=(NA+255)/256;
    for(int ri=0;ri<6;ri++){
        int nr=nr_list[ri];
        int rblk=(nr+255)/256;
        Agent*da,*ha,*db,*hb;Resource*dr;
        cudaMalloc(&da,NA*sizeof(Agent));cudaMalloc(&db,NA*sizeof(Agent));cudaMalloc(&dr,nr*sizeof(Resource));
        ...
        printf("A/R=%3d:1  res=%3d  spec=%.1f ctrl=%.1f ratio=%.2fx (%+.0f%%)\n",
            NA/nr,nr,fs,fc,ratio,(ratio-1)*100);
