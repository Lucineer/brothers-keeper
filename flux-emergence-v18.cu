/* flux-emergence-v18.cu — Energy Transfer (Altruism/Cooperation)
   Novel: agents can donate energy to same-arch neighbors who are low.
   "Collectors" with high energy share with "explorers" who burn energy searching.
   Prediction: energy sharing improves group fitness by enabling risky exploration. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int enable_share){
    ...
    for(int k=0;k<32;k++){
        ...
        /* v18: ENERGY SHARING */
        if(enable_share&&dist<.04f&&a[j].arch==ag->arch){
            /* High energy shares with low energy neighbors */
            if(ag->energy>.7f&&a[j].energy<.3f){
                float give=(ag->energy-a[j].energy)*0.1f;
                ag->energy-=give; a[j].energy+=give*0.95f; /* 5% loss */
            }
        }
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v18 — Energy Transfer (Altruism)\n");
    printf("  High-energy agents share with low-energy same-arch neighbors\n");
    printf("  A: share enabled  B: no share  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_share=0,f_noshare=0,f_ctrl=0;
    ...
    for(int e=0;e<5;e++){
        ...
        float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_share+=f;
        ...
        f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_noshare+=f;
    ...
    f_share/=5;f_noshare/=5;f_ctrl/=5;
    float r_share=(f_ctrl>.01f)?f_share/f_ctrl:1;
    float r_noshare=(f_ctrl>.01f)?f_noshare/f_ctrl:1;
    float boost=(f_noshare>.01f)?f_share/f_noshare:1;
    
    printf("═══════════════════════════════════════════════════════\n");
    printf("  With share:    %.1f (%.2fx vs ctrl)\n",f_share,r_share);
    printf("  Without share: %.1f (%.2fx vs ctrl)\n",f_noshare,r_noshare);
    printf("  Control:       %.1f\n",f_ctrl);
    printf("  Share boost:   %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"ALTRUISM HELPS":"NO EFFECT");
    printf("═══════════════════════════════════════════════════════\n");
