/* flux-emergence-v20.cu — Hierarchical Control
   Novel: the highest-fitness agent in each archetype becomes a "leader".
   Leaders broadcast movement targets to nearby same-arch followers.
   Prediction: hierarchy improves coordination by 15%+. */
...
/* Find leader (highest fitness) per archetype and broadcast target */
__global__ void find_leaders(Agent*a,int na,Resource*r,int nr){
    __shared__ float leader_x[N_ARCH],leader_y[N_ARCH];
    __shared__ float leader_fit[N_ARCH];
    int tid=threadIdx.x;
    /* Each thread finds local best for its archetype */
    float my_best=0;float my_x=0,my_y=0;int my_arch=a[blockIdx.x*blockDim.x+tid].arch;
    for(int j=0;j<na;j++){
        if(a[j].arch==my_arch&&a[j].fitness>my_best){
            /* Find nearest resource for this candidate */
            float bd=1e10f;int br=-1;
            for(int k=0;k<nr;k++){if(r[k].collected)continue;float dx=r[k].x-a[j].x,dy=r[k].y-a[j].y,d=sqrtf(dx*dx+dy*dy);if(d<bd){bd=d;br=k;}}
            if(br>=0){my_best=a[j].fitness;my_x=r[br].x;my_y=r[br].y;}
        }
    }
    if(tid<N_ARCH){leader_x[tid]=0;leader_y[tid]=0;leader_fit[tid]=0;}
    __syncthreads();
    /* Reduce: each arch thread collects from all threads */
    if(tid<N_ARCH){
        /* Scan a subset for this archetype */
        for(int j=0;j<na;j+=blockDim.x){
            int idx=j+tid;if(idx>=na)continue;
            if(a[idx].arch==tid&&a[idx].fitness>leader_fit[tid]){
                float bd=1e10f;int br=-1;
                for(int k=0;k<nr;k++){if(r[k].collected)continue;float dx=r[k].x-a[idx].x,dy=r[k].y-a[idx].y,d=sqrtf(dx*dx+dy*dy);if(d<bd){bd=d;br=k;}}
                if(br>=0){leader_fit[tid]=a[idx].fitness;leader_x[tid]=r[br].x;leader_y[tid]=r[br].y;}
            }
        }
    }
    __syncthreads();
    /* Each thread applies leader influence */
    int i=blockIdx.x*blockDim.x+tid;if(i>=na)return;
    float lx=leader_x[a[i].arch],ly=leader_y[a[i].arch];
    if(lx>0||ly>0){
        float dx=lx-a[i].x,dy=ly-a[i].y,d=sqrtf(dx*dx+dy*dy);
        if(d>.01f){
            float strength=0.003f*a[i].role[2]; /* communicators follow stronger */
            a[i].vx+=(dx/d)*strength;
            a[i].vy+=(dy/d)*strength;
        }
    }
}

__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int hierarchy){
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v20 — Hierarchical Control\n");
    printf("  Highest-fitness agent per archetype directs followers\n");
    printf("  A: with hierarchy  B: flat  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_hier=0,f_flat=0,f_ctrl=0;
    for(int e=0;e<5;e++){
        init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){if(t>50&&t%10==0)find_leaders<<<blk,256>>>(da,NA,dr,NR);tick_s<<<blk,256>>>(da,dr,NA,NR,t,250,1);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_hier+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_flat+=f;
        ...
        printf("Exp %d: hier=%.1f flat=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_hier/=5;f_flat/=5;f_ctrl/=5;
    float rh=(f_ctrl>.01f)?f_hier/f_ctrl:1,rf=(f_ctrl>.01f)?f_flat/f_ctrl:1;
    float boost=(f_flat>.01f)?f_hier/f_flat:1;
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Hierarchy: %.1f (%.2fx ctrl)\n",f_hier,rh);
    printf("  Flat:      %.1f (%.2fx ctrl)\n",f_flat,rf);
    printf("  Control:   %.1f\n",f_ctrl);
    printf("  Boost:     %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.15?"HIERARCHY WINS":boost>1.05?"MARGINAL":"NO EFFECT");
    printf("═══════════════════════════════════════════════════════\n");
