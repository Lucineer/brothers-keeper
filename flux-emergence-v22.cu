/* flux-emergence-v22.cu — Role Mutation + Selection (Evolution)
   Novel: each tick, roles mutate by tiny random amounts.
   High-fitness agents reproduce: low-fitness agents copy roles from nearby high-fitness ones.
   Prediction: evolution finds optimal role distributions faster than fixed initialization. */
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int evolve){
    ...
    for(int k=0;k<32;k++){int j=lcg(&ag->rng)%na;if(j==i)continue;float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);if(dist>=.06f)continue;ag->interactions++;
        ...
        /* v22: EVOLUTION — low fitness agents copy from high fitness neighbors */
        if(evolve&&dist<.04f&&a[j].fitness>ag->fitness*1.5f){
            for(int r=0;r<4;r++)
                ag->role[r]+=(a[j].role[r]-ag->role[r])*0.02f; /* gradual adoption */
        }
    ...
    /* v22: MUTATION — tiny random changes each tick */
    if(evolve){
        for(int r=0;r<4;r++)
            ag->role[r]+=(lcgf(&ag->rng)-.5f)*0.001f;
    }
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v22 — Evolution (Mutation + Selection)\n");
    printf("  Roles mutate + low-fitness agents copy from successful neighbors\n");
    printf("  A: evolve  B: fixed  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_evo=0,f_fix=0,f_ctrl=0,sp_evo=0,sp_fix=0;
    ...
    for(int e=0;e<5;e++){
        ...
        float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_evo+=f;sp_evo+=spec(ha,NA);
        ...
        f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_fix+=f;sp_fix+=spec(hb,NA);
        ...
        printf("Exp %d: evo=%.1f fix=%.1f ctrl=%.1f sp_e=%.3f sp_f=%.3f\n",e+1,f/NA,f/NA,f/NA,spec(ha,NA),spec(hb,NA));
    }
    f_evo/=5;f_fix/=5;f_ctrl/=5;sp_evo/=5;sp_fix/=5;
    float r_evo=(f_ctrl>.01f)?f_evo/f_ctrl:1;
    float r_fix=(f_ctrl>.01f)?f_fix/f_ctrl:1;
    float boost=(f_fix>.01f)?f_evo/f_fix:1;
    
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Evolved: %.1f (%.2fx ctrl, spec=%.3f)\n",f_evo,r_evo,sp_evo);
    printf("  Fixed:   %.1f (%.2fx ctrl, spec=%.3f)\n",f_fix,r_fix,sp_fix);
    printf("  Control: %.1f\n",f_ctrl);
    printf("  Evolution boost: %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Spec change: %.3f → %.3f (%s)\n",sp_fix,sp_evo,sp_evo>sp_fix?"HIGHER":"LOWER");
    printf("  Verdict: %s\n",boost>1.1?"EVOLUTION WINS":boost<0.95?"EVOLUTION HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
