/* flux-emergence-v23.cu — Birth/Death Lifecycle
   Novel: agents have a lifespan (300 ticks). When they die, a new agent is born
   inheriting roles from the fittest nearby agent (with mutation).
   Tests if generational turnover improves fitness over static populations.
   Prediction: turnover refreshes strategies, 10%+ improvement. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,group,age;float tip_x,tip_y,tip_val;unsigned int rng;}Agent;
typedef struct{float x,y,value;int collected;}Resource;

__global__ void init_a(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].age=(int)(lcgf(&a[i].rng)*LIFESPAN*.5f);a[i].tip_x=a[i].tip_y=a[i].tip_val=0;for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}

__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].age=(int)(lcgf(&a[i].rng)*LIFESPAN*.5f);a[i].tip_x=a[i].tip_y=a[i].tip_val=0;for(int r=0;r<4;r++)a[i].role[r]=.25f;}
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int lifecycle){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];
    
    /* v23: LIFECYCLE — death and rebirth */
    if(lifecycle&&ag->age>=LIFESPAN){
        /* Find fittest nearby same-arch agent to inherit from */
        float best_fit=0;int best_j=-1;
        for(int k=0;k<64;k++){
            int j=lcg(&ag->rng)%na;
            if(j==i||a[j].arch!=ag->arch)continue;
            if(a[j].fitness>best_fit){best_fit=a[j].fitness;best_j=j;}
        }
        if(best_j>=0){
            for(int r=0;r<4;r++)
                ag->role[r]=a[best_j].role[r]+(lcgf(&ag->rng)-.5f)*0.05f; /* inherit + mutate */
        }
        /* Clamp and rebirth */
        for(int r=0;r<4;r++){if(ag->role[r]<0)ag->role[r]=0;if(ag->role[r]>1)ag->role[r]=1;}
        ag->x=lcgf(&ag->rng);ag->y=lcgf(&ag->rng);
        ag->vx=ag->vy=0;ag->energy=.5f;ag->age=0;
        ag->fitness=0;ag->res_held=0;ag->tip_val=0;
        return; /* skip this tick for newborn */
    }
    ag->age++;
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v23 — Birth/Death Lifecycle\n");
    printf("  Agents die after 300 ticks, reborn inheriting fittest neighbor's roles\n");
    printf("  A: lifecycle  B: immortal  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_life=0,f_immortal=0,f_ctrl=0;
    
    for(int e=0;e<5;e++){
        init_a<<<blk,256>>>(da,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(da,dr,NA,NR,t,MAXT/2,1);cudaDeviceSynchronize();}
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_life+=f;
        
        init_a<<<blk,256>>>(db,NA);init_r<<<rblk,256>>>(dr,NR);cudaDeviceSynchronize();
        for(int t=0;t<MAXT;t++){tick_s<<<blk,256>>>(db,dr,NA,NR,t,MAXT/2,0);cudaDeviceSynchronize();}
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_immortal+=f;
        ...
        /* Count deaths */
        int deaths=0;for(int i=0;i<NA;i++)if(ha[i].age<100)deaths++;
        printf("Exp %d: life=%.1f immortal=%.1f ctrl=%.1f deaths~%d\n",e+1,f/NA,f/NA,f/NA,deaths);
    }
    f_life/=5;f_immortal/=5;f_ctrl/=5;
    float r_life=(f_ctrl>.01f)?f_life/f_ctrl:1;
    float r_imm=(f_ctrl>.01f)?f_immortal/f_ctrl:1;
    float boost=(f_immortal>.01f)?f_life/f_immortal:1;
    
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Lifecycle:  %.1f (%.2fx ctrl)\n",f_life,r_life);
    printf("  Immortal:   %.1f (%.2fx ctrl)\n",f_immortal,r_imm);
    printf("  Control:    %.1f\n",f_ctrl);
    printf("  Lifecycle boost: %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"LIFECYCLE WINS":boost<0.95?"LIFECYCLE HURTS":"NO DIFFERENCE");
    printf("═══════════════════════════════════════════════════════\n");
