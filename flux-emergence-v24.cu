/* flux-emergence-v24.cu — Signaling (Attract/Repel)
   Novel: collectors emit an ATTRACT signal when they find resources.
   Other archetypes are pulled toward the signal. Defenders emit REPEL.
   Tests if directional signaling improves coordination beyond comms.
   Prediction: signals improve group convergence on resources. */
...
typedef struct{float x,y,vx,vy,energy,role[4],fitness;int arch,res_held,interactions,group;float tip_x,tip_y,tip_val;float signal_x,signal_y,signal_str,signal_type;unsigned int rng;}Agent;
typedef struct{float x,y,value;int collected;}Resource;

__global__ void init_a(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+17);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].signal_x=a[i].signal_y=a[i].signal_str=a[i].signal_type=0;for(int r=0;r<4;r++){float b=(r==a[i].arch)?.7f:.1f;a[i].role[r]=b+(lcgf(&a[i].rng)-.5f)*.4f;}}

__global__ void init_c(Agent*a,int n){int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=n)return;a[i].rng=(unsigned int)(i*2654435761u+99917);a[i].x=lcgf(&a[i].rng);a[i].y=lcgf(&a[i].rng);a[i].vx=a[i].vy=0;a[i].energy=.5f+lcgf(&a[i].rng)*.5f;a[i].arch=i%N_ARCH;a[i].fitness=a[i].res_held=a[i].interactions=0;a[i].group=-1;a[i].tip_x=a[i].tip_y=a[i].tip_val=0;a[i].signal_x=a[i].signal_y=a[i].signal_str=a[i].signal_type=0;for(int r=0;r<4;r++)a[i].role[r]=.25f;}
...
__global__ void tick_s(Agent*a,Resource*r,int na,int nr,int t,int pt,int signaling){
    int i=blockIdx.x*blockDim.x+threadIdx.x;if(i>=na)return;
    Agent*ag=&a[i];
    float ep=ag->role[0],cp=ag->role[1],cm=ag->role[2],df=ag->role[3];
    float det=.03f+ep*.04f,grab=.02f+cp*.02f;
    
    /* Respond to signals from neighbors */
    float sig_dx=0,sig_dy=0;
    if(signaling){
        for(int k=0;k<16;k++){
            int j=lcg(&ag->rng)%na;if(j==i)continue;
            float dx=a[j].x-ag->x,dy=a[j].y-ag->y,dist=sqrtf(dx*dx+dy*dy);
            if(dist>=.1f||a[j].signal_str<.01f)continue;
            float strength=a[j].signal_str/(dist*dist+.01f); /* inverse square */
            if(a[j].signal_type>0)/* attract */{sig_dx+=(dx/dist)*strength;sig_dy+=(dy/dist)*strength;}
            else/* repel */{sig_dx-=(dx/dist)*strength*.5f;sig_dy-=(dy/dist)*strength*.5f;}
        }
        ag->vx+=sig_dx*0.001f;ag->vy+=sig_dy*0.001f;
    }
    ...
    if(br>=0&&bd<grab){
        ...
        /* Emit ATTRACT signal toward this resource */
        if(signaling){ag->signal_x=r[br].x;ag->signal_y=r[br].y;ag->signal_str=cp;ag->signal_type=1.0f;}
        ...
        /* Defenders emit REPEL */
        if(signaling&&ag->arch==3){ag->signal_x=ag->x;ag->signal_y=ag->y;ag->signal_str=df*.5f;ag->signal_type=-1.0f;}
    }
    ag->x=fmodf(ag->x+ag->vx+1,1);ag->y=fmodf(ag->y+ag->vy+1,1);
    if(signaling)ag->signal_str*=.8f; /* signal decay */
    ...
    if(t==pt){ag->energy*=(1-.5f*(1-df*.5f));ag->x=lcgf(&ag->rng);ag->y=lcgf(&ag->rng);ag->vx=ag->vy=0;ag->tip_val=0;ag->signal_str=0;}
}
...
int main(){
    printf("═══════════════════════════════════════════════════════\n");
    printf("  FLUX v24 — Signaling (Attract/Repel)\n");
    printf("  Collectors emit attract near resources, defenders emit repel\n");
    printf("  A: signals  B: no signals  C: control\n");
    printf("═══════════════════════════════════════════════════════\n\n");
    ...
    float f_sig=0,f_nosig=0,f_ctrl=0;
    ...
    for(int e=0;e<5;e++){
        ...
        cudaMemcpy(ha,da,NA*sizeof(Agent),cudaMemcpyDeviceToHost);float f=0;for(int i=0;i<NA;i++)f+=ha[i].fitness;f_sig+=f;
        ...
        cudaMemcpy(hb,db,NA*sizeof(Agent),cudaMemcpyDeviceToHost);f=0;for(int i=0;i<NA;i++)f+=hb[i].fitness;f_nosig+=f;
        ...
        printf("Exp %d: sig=%.1f nosig=%.1f ctrl=%.1f\n",e+1,f/NA,f/NA,f/NA);
    }
    f_sig/=5;f_nosig/=5;f_ctrl/=5;
    float r_sig=(f_ctrl>.01f)?f_sig/f_ctrl:1;
    float r_nosig=(f_ctrl>.01f)?f_nosig/f_ctrl:1;
    float boost=(f_nosig>.01f)?f_sig/f_nosig:1;
    
    printf("\n═══════════════════════════════════════════════════════\n");
    printf("  Signals:  %.1f (%.2fx ctrl)\n",f_sig,r_sig);
    printf("  No sigs:  %.1f (%.2fx ctrl)\n",f_nosig,r_nosig);
    printf("  Control:  %.1f\n",f_ctrl);
    printf("  Boost:    %.2fx (%+.0f%%)\n",boost,(boost-1)*100);
    printf("  Verdict: %s\n",boost>1.1?"SIGNALS HELP":boost<0.95?"SIGNALS HURT":"NO EFFECT");
    printf("═══════════════════════════════════════════════════════\n");
